...
RSI (façon MACD appliqué au RSI plutôt qu'au prix), filtrée par un
niveau plancher du RSI, sur les cours SRD déjà récupérés. Recherche
exhaustive / grid search, puis génération automatique du détail des
trades et des graphiques pour la meilleure combinaison trouvée.

Stratégie testée, long-only, une position à la fois par valeur :
- Achat : le RSI est AU-DESSUS du niveau d'entrée ET la moyenne mobile
  exponentielle COURTE du RSI passe au-dessus de la moyenne mobile
  exponentielle LONGUE du RSI.
- Vente : la moyenne courte repasse sous la moyenne longue (pas de
  filtre de niveau à la sortie).
...
⚠️ Avertissement surapprentissage (overfitting) : 1 404 combinaisons
...
def find_trades(
    name: str, closes: pd.Series, rsi: pd.Series, ema_short: pd.Series, ema_long: pd.Series, entry_min: int
) -> list:
    ...
    comparer explicitement au point précédent. Le filtre de niveau ne
    s'applique qu'à l'entrée, pas à la sortie.
    """
    in_position = False
    ...
    for dt, price in closes.items():
        ...
        if pd.isna(r) or pd.isna(es) or pd.isna(el):
            continue
        if not in_position and r > entry_min and es > el:
            in_position = True
...
def plot_ticker(
    ...
    ax_rsi.axhline(entry_min, color="grey", linestyle="--", linewidth=0.8, label=f"Seuil entrée {entry_min}")
...
def run_grid_search(tickers: dict) -> pd.DataFrame:
    ...
    n_combos = len(rsi_periods) * len(ema_pairs) * len(entry_levels)
    print(
        f"{n_combos} combinaisons à tester "
        f"({len(rsi_periods)} périodes RSI x {len(ema_pairs)} paires d'EMA courte/longue "
        f"x {len(entry_levels)} niveaux d'entrée) — environ 2 à 3 minutes sur 30 valeurs...\n"
    )
    ...
    for rsi_period, (short_period, long_period), entry_min in product(rsi_periods, ema_pairs, entry_levels):
        ...
        for name, closes in tickers.items():
            ...
            for t in find_trades(name, closes, rsi, ema_short, ema_long, entry_min):
                if t["Statut"] == "Clôturé":
...
def run_full_backtest(
    tickers: dict, rsi_period: int, short_period: int, long_period: int, entry_min: int, charts_dir: Path
):
    ...
    for name, closes in tickers.items():
        ...
        trades = find_trades(name, closes, rsi, ema_short, ema_long, entry_min)
        ...
        plot_ticker(
            name, closes, rsi, ema_short, ema_long, trades, charts_dir,
            rsi_period, short_period, long_period, entry_min,
        )
...
def main() -> None:
    ...
    entry_min = int(best["Niveau entrée RSI"])
    ...
    print(
        ...
        f"- Niveau d'entrée RSI  : {entry_min}\n"
    ...
    summary, trades_df, total_closed, total_latent = run_full_backtest(
        tickers, rsi_period, short_period, long_period, entry_min, charts_dir
    )
