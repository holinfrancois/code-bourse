...
- Vente : stop suiveur classique. On garde le plus haut cours de
  ...
  sous ce plus haut, on vend. L'ancienne sortie par croisement inverse
  des moyennes est supprimée.

  Note : une version précédente utilisait une fenêtre glissante bornée
  (un "plus haut sur les N dernières séances") pilotable par une
  longueur de période. Testée en réel, elle s'est révélée dangereuse
  sur les baisses lentes et progressives avec une période courte : le
  plus haut de référence redescendait avec le cours au lieu de rester
  fixé au sommet, et le stop ne protégeait quasiment plus (position
  restée ouverte 121 semaines à -16 % de latent sur un cas testé). Le
  stop suiveur classique depuis l'entrée élimine ce problème par
  construction. Le paramètre "longueur de période" a donc été retiré.
...
deux EMA, seuil, perte du stop) et tracent le niveau du stop suiveur
pendant les positions ouvertes.

⚠️ Avertissement surapprentissage (overfitting) : 11 232 combinaisons
...
def simulate_fast(closes_arr, rsi_arr, es_arr, el_arr, entry_max: int, stop_loss_pct: float):
...
def plot_ticker(
    ...
    ax_price.set_title(
        ...
        f"/ seuil RSI < {entry_max}  /  stop suiveur {stop_loss_pct}% depuis l'entrée"
    )
...
def run_grid_search(tickers: dict) -> pd.DataFrame:
    ...
    n_combos = len(rsi_periods) * len(ema_pairs) * len(entry_levels) * len(stop_losses)
    print(
        f"{n_combos} combinaisons à tester "
        f"({len(rsi_periods)} périodes RSI x {len(ema_pairs)} paires d'EMA x "
        f"{len(entry_levels)} niveaux d'entrée x {len(stop_losses)} pertes stop) "
...
def run_full_backtest(
    ...
    for name, closes in tickers.items():
        ...
        trades, stop_level_series = find_trades(
            name, closes, rsi, ema_short, ema_long, entry_max, stop_loss_pct
        )
        ...
        plot_ticker(
            ...
            rsi_period, short_period, long_period, entry_max, stop_loss_pct,
        )
...
def main() -> None:
    ...
    summary, trades_df, total_closed, total_latent = run_full_backtest(
        tickers, rsi_period, short_period, long_period, entry_max, stop_loss_pct, charts_dir
    )
