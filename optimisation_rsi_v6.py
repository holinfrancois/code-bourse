...
niveau plafond du RSI, sur les cours SRD déjà récupérés. Recherche
...
def find_trades(
    name: str, closes: pd.Series, rsi: pd.Series, ema_short: pd.Series, ema_long: pd.Series, entry_max: int
) -> list:
    ...
    for dt, price in closes.items():
        ...
        if pd.isna(r) or pd.isna(es) or pd.isna(el):
            continue
        if not in_position and r < entry_max and es > el:
            in_position = True
...
def run_grid_search(tickers: dict) -> pd.DataFrame:
    ...
    for rsi_period, (short_period, long_period), entry_max in product(rsi_periods, ema_pairs, entry_levels):
        ...
        for name, closes in tickers.items():
            ...
            ticker_closed = 0.0
            for t in find_trades(name, closes, rsi, ema_short, ema_long, entry_max):
                if t["Statut"] == "Clôturé":
                    closed_sum += t["Rendement (%)"]
                    ticker_closed += t["Rendement (%)"]
                    n_closed += 1
                    n_wins += t["Rendement (%)"] > 0
                else:
                    latent_sum += t["Rendement (%)"]
            per_ticker_closed[name] = ticker_closed
...
def run_full_backtest(
    tickers: dict, rsi_period: int, short_period: int, long_period: int, entry_max: int, charts_dir: Path
):
    ...
    for name, closes in tickers.items():
        ...
        trades = find_trades(name, closes, rsi, ema_short, ema_long, entry_max)
        ...
        plot_ticker(
            ...
            rsi_period, short_period, long_period, entry_max,
        )
...
def main() -> None:
    ...
    summary, trades_df, total_closed, total_latent = run_full_backtest(
        tickers, rsi_period, short_period, long_period, entry_max, charts_dir
    )
