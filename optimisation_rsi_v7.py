...
par un niveau plafond du RSI, avec une SORTIE PAR STOP SUIVEUR (au lieu
du croisement inverse des moyennes), sur les cours SRD déjà récupérés.
Recherche exhaustive / grid search, puis génération automatique du
détail des trades et des graphiques pour la meilleure combinaison
trouvée.

Stratégie testée, long-only, une position à la fois par valeur :
- Achat : le RSI est sous le niveau d'entrée ET la moyenne mobile
  exponentielle COURTE du RSI passe au-dessus de la moyenne mobile
  exponentielle LONGUE du RSI (entrée inchangée par rapport aux
  versions précédentes).
- Vente : stop suiveur. On garde le plus haut cours de clôture sur les
  N dernières séances (fenêtre glissante, N = "longueur de période") ;
  dès que la clôture retombe de plus de X % sous ce plus haut, on
  vend. L'ancienne sortie par croisement inverse des moyennes est
  supprimée.
...
- Niveau d'entrée du RSI  : 30 à 70 par pas de 5
- Perte du stop suiveur   : 6 % à 20 % par pas de 2
- Longueur de période du stop suiveur : 2 à 10 par pas de 2
(56 160 combinaisons)
...
deux EMA, seuil, perte et longueur du stop) et tracent le niveau du
stop suiveur pendant les positions ouvertes.

⚠️ Avertissement surapprentissage (overfitting) : 56 160 combinaisons
testées sur seulement ~5 ans d'historique et une trentaine de valeurs,
c'est un terrain massivement propice au surapprentissage — la
meilleure combinaison sur ce passé précis n'a aucune garantie de bien
se comporter à l'avenir.
...
- graphiques_stop_rsi_meilleure_config_<horodatage>/ : un PNG par
  ...
  avec la ligne du stop suiveur pendant les positions ouvertes.
"""
...
ENTRY_LEVELS = range(30, 71, 5)
STOP_LOSS_PCTS = range(6, 21, 2)
STOP_PERIODS = range(2, 11, 2)
...
def find_trades(
    ...
    Stop suiveur : pendant qu'on est en position, on garde le plus haut
    cours de clôture sur les `stop_period` dernières séances (fenêtre
    glissante bornée, pas le plus haut depuis l'entrée sans limite) et
    on vend dès que la clôture repasse sous ce plus haut réduit de
    `stop_loss_pct` %.
    """
    in_position = False
    ...
    for dt, price in closes.items():
        ...
        es = ema_short.get(dt)
        el = ema_long.get(dt)
        if pd.isna(r) or pd.isna(es) or pd.isna(el):
            continue

        if not in_position:
            if r < entry_max and es > el:
                in_position = True
                entry_date, entry_price = dt, price
                window = [price]
        else:
            window.append(price)
            if len(window) > stop_period:
                window.pop(0)
...
def simulate_fast(closes_arr, rsi_arr, es_arr, el_arr, entry_max: int, stop_loss_pct: float, stop_period: int):
    ...
    for i in range(n):
        ...
        es = es_arr[i]
        el = el_arr[i]
        if r != r or es != es or el != el:  # plus rapide que pd.isna/np.isnan ici
            continue
        price = closes_arr[i]
...
def plot_ticker(
    ...
    ax_price.set_title(
        ...
        f"/ seuil RSI < {entry_max}  /  stop suiveur {stop_loss_pct}% sur {stop_period} séances"
    )
    ...
    ax_rsi.axhline(entry_max, color="grey", linestyle="--", linewidth=0.8, label=f"Seuil entrée {entry_max}")
...
def run_grid_search(tickers: dict) -> pd.DataFrame:
    ...
    stop_periods = list(STOP_PERIODS)
    ema_pairs = [(s, l) for s in short_periods for l in long_periods if s < l]
    n_combos = len(rsi_periods) * len(ema_pairs) * len(entry_levels) * len(stop_losses) * len(stop_periods)
    print(
        f"{n_combos} combinaisons à tester "
        f"({len(rsi_periods)} périodes RSI x {len(ema_pairs)} paires d'EMA x "
        f"{len(entry_levels)} niveaux d'entrée x {len(stop_losses)} pertes stop x "
        f"{len(stop_periods)} longueurs de stop) — environ 4 minutes sur 30 valeurs...\n"
    )
    ...
    rsi_arrs = {rp: {name: s.values for name, s in d.items()} for rp, d in rsi_cache.items()}
    ema_arrs = {key: {name: s.values for name, s in d.items()} for key, d in ema_cache.items()}

    results = []
    done = 0
    last_rsi_period = None
    for rsi_period, (short_period, long_period), entry_max in product(rsi_periods, ema_pairs, entry_levels):
        ...
        for stop_loss_pct, stop_period in product(stop_losses, stop_periods):
            ...
            results.append(
                {
                    ...
                    "Niveau entrée RSI": entry_max,
                    "Stop perte (%)": stop_loss_pct,
                    "Stop période": stop_period,
...
def run_full_backtest(
    ...
    for name, closes in tickers.items():
        ...
        trades, stop_level_series = find_trades(
            name, closes, rsi, ema_short, ema_long, entry_max, stop_loss_pct, stop_period
        )
        ...
        plot_ticker(
            name, closes, rsi, ema_short, ema_long, trades, stop_level_series, charts_dir,
            rsi_period, short_period, long_period, entry_max, stop_loss_pct, stop_period,
        )
...
def main() -> None:
    ...
    stop_period = int(best["Stop période"])
    ...
    print(
        ...
        f"- Perte du stop suiveur     : {stop_loss_pct} %\n"
        f"- Longueur du stop suiveur  : {stop_period} séances\n"
    ...
    summary, trades_df, total_closed, total_latent = run_full_backtest(
        tickers, rsi_period, short_period, long_period, entry_max, stop_loss_pct, stop_period, charts_dir
    )
